...
**/
    ...
    if((*RootPtr) == NULL) //pokud neexistuje udelam novy uzel
    {
        tBSTNodePtr tmp; //promena pro nej
        if((tmp = malloc(sizeof(struct tBSTNode))) == NULL) //alokace dobre nebo ne?
            return;
        tmp->Key = K; //vlozim klic
        tmp->BSTNodeCont = Content; //vlozim obsah
        tmp->LPtr = NULL; //jelikoz byl prvni tak zatim nema zadne syny
        tmp->RPtr = NULL; //jelikoz byl prvni tak zatim nema zadne syny
        *RootPtr = tmp;
    }
    else if((*RootPtr)->Key < K) //pokud uzel existuje, ceknu jestli je K > nebo mensi nez Key a volam prislusnou funkci pro dalsi uzel (rekurzivni)
        BSTInsert(&(*RootPtr)->RPtr, K, Content);
    else if((*RootPtr)->Key > K) //pokud uzel existuje, ceknu jestli je K > nebo mensi nez Key a volam prislusnou funkci pro dalsi uzel (rekurzivni)
        BSTInsert(&(*RootPtr)->LPtr, K, Content);
    else
        (*RootPtr)->BSTNodeCont = Content; //pokud se klice rovnaji, tak aktualizuju hodnotu
...
**/
    ...
    if((*RootPtr)->RPtr) //pokud existuje pravy, volej funkci rekurzivne
    {
        ReplaceByRightmost(PtrReplaced, &(*RootPtr)->RPtr); //rekurze pro pravy podstrom
    }
    else //pokud neexistuje tak
    {
        PtrReplaced->Key = (*RootPtr)->Key; //nahrad klic uzlu co je aktualizovan
        PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont; //nahrad hodnotu uzlu co je aktualizovan
        PtrReplaced = *RootPtr; //PtrReplaced ukazuje na nejpravejsi uzel
        *RootPtr = (*RootPtr)->LPtr; //RootPtr ukazuje na levy uzel uzel nejpravejsiho podstromu
        free(PtrReplaced); //uvolni nejpravejsi uzel
    }
}
...
**/
    if((*RootPtr) == NULL) //jestlize vubec existuje onen koren
        return;
    if((*RootPtr)->Key == K) //jestlize existuje a key je roven klici korene
    {
        tBSTNodePtr tmp = *RootPtr; //ulozim si koren do pomocne promene
        if((*RootPtr)->LPtr == NULL) //jestlize nema levy podstrom tak ho nahrad pravym podstromem
        {
            *RootPtr = (*RootPtr)->RPtr;
            free(tmp);
        }
        else if((*RootPtr)->RPtr == NULL) //jestlize nema pravy podstrom nahrad ho levym podstromem
        {
            *RootPtr = (*RootPtr)->LPtr;
            free(tmp);
        }
        else //jestlize ma oba podstromy, nahrad ho nejpravejsim podstromem leveho podstromu
        {
    ...
    {
        BSTDelete(&(*RootPtr)->LPtr, K); //kdyz klic neni roven tak rekurzivne zavolej pro levy podstrom
    }
    else
    {
        BSTDelete(&(*RootPtr)->RPtr, K); //kdyz klic neni roven tak rekurzivne zavolej pro pravy podstrom
    }
}
...
**/
    ...
    if((*RootPtr) != NULL) //pokud neco obsahuje
    {
        BSTDispose(&(*RootPtr)->LPtr); //zrus jeho pravy podstrom
        BSTDispose(&(*RootPtr)->RPtr); //levypodstrom
        free(*RootPtr); //vymaz koren
        *RootPtr = NULL; //nastav RootPtr na null viz inicializace
    }
    else //pokud nic neobsahuje, vrat se a nedelej nic
    {
        return;
    }
}
